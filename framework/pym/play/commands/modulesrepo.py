...
def build(app, args, env):
    ...
    if version is None:
        version = eval(input("~ What is the module version number? "))
    if fwkMatch is None:
        fwkMatch = eval(input("~ What are the playframework versions required? "))
        ...
        if mod['name'] == module:
            for v in mod['versions']:
                if version is None and v['isDefault']:
                    ...
                    ok = eval(input('~ Do you want to install this version (y/n)? '))
                    if not ok == 'y':
                        print('~')
                        sys.exit(-1)
